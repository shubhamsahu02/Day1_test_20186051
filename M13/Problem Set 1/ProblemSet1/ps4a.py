...
#
def get_wordscore(word_entered, len_gth):
    """
...
def play_hand(ha_nd, word_lists, num_ber):
    """
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    cum_score = 0
    while calculate_handlen(ha_nd):
        display_hand(ha_nd)
        in_put = input("Enter a word or insert a '.' to exit: ")
        if in_put == '.':
            print("Your score is: ", cum_score)
            break
        elif is_validword(in_put, ha_nd, word_lists):
            cum_score += get_wordscore(in_put, num_ber)
            print("Your score: ", cum_score)
            ha_nd = update_hand(ha_nd, in_put)
        else:
...
def play_game(word_list):
    ...
    while True:
        play_input = input("Enter n to play with new hand\n\
r to play with previous hand\ne to exit: ")
        if play_input == 'n':
            ...
            if bool(hand):
                ...
                print("No previous hands. Please re-enter n or e")
        elif play_input == 'e':
            print("bye bye")
            break
        else:
